...
if [ "$OPENIMAGES" = true ] ; then
  # openimages_f16_8192 - Gumbel - 376 MB:
  if [ ! -f "$LOC"/vqgan_openimages_f16_8192.yaml ]; then
    curl -L -o "$LOC"/vqgan_openimages_f16_8192.yaml -C - 'https://heibox.uni-heidelberg.de/d/2e5662443a6b4307b470/files/?p=%2Fconfigs%2Fmodel.yaml&dl=1' 
  fi
  if [ ! -f "$LOC"/vqgan_openimages_f16_8192.ckpt ]; then
    curl -L -o "$LOC"/vqgan_openimages_f16_8192.ckpt -C - 'https://heibox.uni-heidelberg.de/d/2e5662443a6b4307b470/files/?p=%2Fckpts%2Flast.ckpt&dl=1'    
  fi
fi
