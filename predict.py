...
class Predictor(cog.Predictor):
    def setup(self):
        ...
        self.perceptor = clip.load(self.args.clip_model, jit=jit)[0].eval().requires_grad_(False).to(self.device)
        cut_size = self.perceptor.visual.input_resolution
        # choose latest Cutout class as default
        self.make_cutouts = MakeCutouts(cut_size, self.args.cutn, self.args, cut_pow=self.args.cut_pow)
    ...
    def predict(self, image, prompts, iterations, display_frequency):
        ...
        for i in range(1, self.args.max_iterations + 1):
            ...
            lossAll = ascend_txt(i, z, self.perceptor, self.args, self.model, self.make_cutouts, pMs)
...
def get_args():
    ...
    vq_parser.add_argument("-ip", "--image_prompts", type=str, help="Image prompts / target image", default=[],
                           dest='image_prompts')
    vq_parser.add_argument("-i", "--iterations", type=int, help="Number of iterations", default=500,
                           dest='max_iterations')
    vq_parser.add_argument("-se", "--save_every", type=int, help="Save image iterations", default=50,
                           dest='display_freq')
    vq_parser.add_argument("-s", "--size", nargs=2, type=int, help="Image size (width height) (default: %(default)s)",
                           dest='size')
    vq_parser.add_argument("-ii", "--init_image", type=str, help="Initial image", default=None, dest='init_image')
    vq_parser.add_argument("-in", "--init_noise", type=str, help="Initial noise image (pixels or gradient)",
                           default=None, dest='init_noise')
    vq_parser.add_argument("-iw", "--init_weight", type=float, help="Initial weight", default=0., dest='init_weight')
    vq_parser.add_argument("-m", "--clip_model", type=str, help="CLIP model (e.g. ViT-B/32, ViT-B/16)",
    ...
    vq_parser.add_argument("-npw", "--noise_prompt_weights", nargs="*", type=float, help="Noise prompt weights",
                           default=[], dest='noise_prompt_weights')
    ...
    vq_parser.add_argument("-zvid", "--zoom_video", action='store_true', help="Create zoom video?",
    ...
    vq_parser.add_argument("-zsx", "--zoom_shift_x", type=int, help="Zoom shift x (left/right) amount in pixels",
                           default=0, dest='zoom_shift_x')
    vq_parser.add_argument("-zsy", "--zoom_shift_y", type=int, help="Zoom shift y (up/down) amount in pixels",
                           default=0, dest='zoom_shift_y')
    ...
    vq_parser.add_argument("-ofps", "--output_video_fps", type=float,
                           ...
                           default=30, dest='output_video_fps')
    ...
    vq_parser.add_argument("-aug", "--augments", nargs='+', action='append', type=str,
                           ...
                           help="Enabled augments (latest vut method only)", default=[['Af', 'Pe', 'Ji', 'Er']],
                           dest='augments')
    vq_parser.add_argument("-vsd", "--video_style_dir", type=str, help="Directory with video frames to style",
                           default=None, dest='video_style_dir')
    ...
    def __init__(self, cut_size, cutn, args, cut_pow=1.):
        ...
        for item in args.augments[0]:
            if item == 'Ji':
                ...
                augment_list.append(K.RandomAffine(degrees=15, translate=0.1, shear=5, p=0.7, padding_mode='zeros',
                                                   keepdim=True))  # border, reflection, zeros
            elif item == 'Et':
