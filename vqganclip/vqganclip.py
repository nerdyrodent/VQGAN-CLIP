...
def get_filename(output_filename, i):
    p, ext = os.path.splitext(output_filename)
    return f"{p}_{i}{ext}"
...
class MakeCutouts(nn.Module):
    def __init__(self, cut_size, cutn, augments, cut_pow=1.0):
        super().__init__()
...
def run(args: argparse.Namespace):
    ...
    if args.cut_method == "latest":
        make_cutouts = MakeCutouts(
            cut_size, args.cutn, args.augments, cut_pow=args.cut_pow
        )
    ...
    def checkin(i, losses):
        ...
        TF.to_pil_image(out[0].cpu()).save(get_filename(args.output, i), pnginfo=info)
                    ...
                    if i % args.zoom_frequency == 0:
                        ...
                        if args.zoom_start <= i:
                            ...
                            pil_image = Image.fromarray(
                                np.array(img).astype("uint8"), "RGB"
                            )

                            # Zoom
                            if args.zoom_scale != 1:
                                pil_image_zoom = zoom_at(
                                    pil_image, sideX / 2, sideY / 2, args.zoom_scale
                                )
                            else:
                            ...
                            if args.zoom_shift_x or args.zoom_shift_y:
                                ...
                                pil_image_zoom = ImageChops.offset(
                                    pil_image_zoom, args.zoom_shift_x, args.zoom_shift_y
                                )
                            ...
                            # Re-encode
                            z, *_ = model.encode(
                                pil_tensor.to(device).unsqueeze(0) * 2 - 1
                            )
                            z_orig = z.clone()
                            z.requires_grad_(True)
